...
instance FromJSON TextPart where
  parseJSON = withObject "TextPart" $ \v -> TextPart
    <$> v .:? "id"
    <*> v .: "text"
    <*> v .:? "type"
...
textPartInput txt = TextPartInput "text" txt
...
instance FromJSON AssistantMessage where
  parseJSON = withObject "AssistantMessage" $ \v -> AssistantMessage
    <$> v .: "id"
    <*> v .: "sessionID"
    <*> v .:? "time"
...
instance FromJSON ProvidersResponse where
  parseJSON = withObject "ProvidersResponse" $ \v -> ProvidersResponse
    <$> v .: "all"
    <*> v .:? "connected"
    <*> v .:? "default"
